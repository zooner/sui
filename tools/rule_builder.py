...
import shutil
import re
import builder.scg_build as scg_build
import builder.defines as defines
...
if __name__ == '__main__':
    ...
    parser.add_option("--bin", dest = "bin",
                      ...
                      metavar = "BIN", default = None)
    parser.add_option("--fs_repo_src", dest = "fs_repo_src",
                      help = "use specified path for fs_repo_src",
                      metavar = "FS_REPO_SRC", default = None)
    parser.add_option("--fs_repo", dest = "fs_repo",
                      help = "use specified path for fs_repo",
                      metavar = "FS_REPO", default = None)
    parser.add_option("--new-sc", dest = "new_sc",
                      help = "use new tools for last version of sc-memory",
                      metavar = "NEW_SC", default = True)
    parser.add_option("--new-style-bin-pairs", dest = "new_style_bin_pairs",
                      help = "use new style for binary pairs (without 1_ and 2_ attributes)",
                      metavar = "NEW_STYLE_BINARY_PAIRS", default = True)
    (options, args) = parser.parse_args()

    if options.fs_repo_src is not None:
        defines.PATH_REPO_SRC = options.fs_repo_src
    if options.fs_repo is not None:
        defines.PATH_REPO_BIN = options.fs_repo

    defines.NEW_SC_MODEL = options.new_sc
    defines.NEW_STYLE_BINARY_PAIRS = options.new_style_bin_pairs
